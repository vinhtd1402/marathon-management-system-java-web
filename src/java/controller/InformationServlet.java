...
public class InformationServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        // Gọi DAO để lấy danh sách sự kiện đã tham gia
        RegistrationDAO dao = new RegistrationDAO();
        List<Registration> regList = dao.getRegistrationsByRunner(acc.getuID());

        // Gửi dữ liệu sang JSP
        request.setAttribute("account", acc);
        request.setAttribute("regList", regList);

        request.getRequestDispatcher("views/infor.jsp").forward(request, response);
    } 
